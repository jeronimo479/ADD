# AD&D 2E Player Reference

## Player Abilities

### Strength

| Ability Score | Hit Prob. | Damage Adj. | Weight Allow. | Max Press | Open Doors | Bend Bars / Lift Gates |
|---------------|-----------|-------------|---------------|-----------|------------|------------------------|
| 3             | -3        | -1          | 5             | 10        | 2          | 0%                     |
| 4-5           | -2        | -1          | 10            | 25        | 3          | 0%                     |
| 6-7           | -1        | None        | 20            | 55        | 4          | 0%                     |
| 8-9           | Normal    | None        | 35            | 90        | 5          | 1%                     |
| 10-11         | Normal    | None        | 40            | 115       | 6          | 2%                     |
| 12-13         | Normal    | None        | 45            | 140       | 7          | 4%                     |
| 14-15         | Normal    | None        | 55            | 170       | 8          | 7%                     |
| 16            | Normal    | +1          | 70            | 195       | 9          | 10%                    |
| 17            | +1        | +1          | 85            | 220       | 10         | 13%                    |
| 18            | +1        | +2          | 110           | 255       | 11         | 16%                    |
| 18/01-50      | +1        | +3          | 135           | 280       | 12         | 20%                    |
| 18/51-75      | +2        | +3          | 160           | 305       | 13         | 25%                    |
| 18/76-90      | +2        | +4          | 185           | 330       | 14         | 30%                    |
| 18/91-99      | +2        | +5          | 235           | 380       | 15 (3)     | 35%                    |
| 18/00         | +3        | +6          | 335           | 480       | 16 (6)     | 40%                    |

> Only Fighters can have exception strength 18/XX% scores.
### Dexterity

| Ability Score | Reaction Adj. | Missile Attack Adj. | Defensive Adj. |
|---------------|---------------|---------------------|----------------|
| 3             | -3            | -3                  | +4             |
| 4             | -2            | -2                  | +3             |
| 5             | -1            | -1                  | +2             |
| 6             | 0             | 0                   | +1             |
| 7-14          | 0             | 0                   |  0             |
| 15            | 0             | 0                   | -1             |
| 16            | +1            | +1                  | -2             |
| 17            | +2            | +2                  | -3             |
| 18            | +2            | +2                  | -4             |
| 19            | +3            | +3                  | -4             |

### Constitution

| Ability Score | Hit Point Adjustment | System Shock | Resurrection Survival | Poison Save | Regeneration |
|---------------|----------------------|--------------|-----------------------|-------------|-------------|
| 3             | -2                   | 35%          | 40%                   | 0           | Nil         |
| 4             | -1                   | 40%          | 45%                   | 0           | Nil         |
| 5             | -1                   | 45%          | 50%                   | 0           | Nil         |
| 6             | -1                   | 50%          | 55%                   | 0           | Nil         |
| 7             | 0                    | 55%          | 60%                   | 0           | Nil         |
| 8             | 0                    | 60%          | 65%                   | 0           | Nil         |
| 9             | 0                    | 65%          | 70%                   | 0           | Nil         |
| 10            | 0                    | 70%          | 75%                   | 0           | Nil         |
| 11            | 0                    | 75%          | 80%                   | 0           | Nil         |
| 12            | 0                    | 80%          | 85%                   | 0           | Nil         |
| 13            | 0                    | 85%          | 90%                   | 0           | Nil         |
| 14            | 0                    | 88%          | 92%                   | 0           | Nil         |
| 15            | +1                   | 90%          | 94%                   | 0           | Nil         |
| 16            | +2                   | 95%          | 96%                   | 0           | Nil         |
| 17            | +2 (+3)\*            | 97%          | 98%                   | 0           | Nil         |
| 18            | +2 (+4)\*            | 99%          | 100%                  | 0           | Nil         |
| 19            | +2 (+5)\*            | 99%          | 100%                  | +1          | Nil         |

\* Parenthetical bonus applies to warriors only. All other classes receive a maximum bonus of +2 per die.

### Intelligence

| Ability Score | # of Languages | Spell Level | Chance to Learn Spell | Max. # of Spells / Level |
|---------------|----------------|-------------|-----------------------|-------------------------:|
| 3             | 1              | —           | —                     | —                        |
| 4             | 1              | —           | —                     | —                        |
| 5             | 1              | —           | —                     | —                        |
| 6             | 1              | —           | —                     | —                        |
| 7             | 1              | —           | —                     | —                        |
| 8             | 1              | —           | —                     | —                        |
| 9             | 2              | 4th         | 35%                   | 6                        |
| 10            | 2              | 5th         | 40%                   | 7                        |
| 11            | 2              | 5th         | 45%                   | 7                        |
| 12            | 3              | 6th         | 50%                   | 7                        |
| 13            | 3              | 6th         | 55%                   | 9                        |
| 14            | 4              | 7th         | 60%                   | 9                        |
| 15            | 4              | 7th         | 65%                   | 11                       |
| 16            | 5              | 8th         | 70%                   | 11                       |
| 17            | 6              | 8th         | 75%                   | 14                       |
| 18            | 7              | 9th         | 85%                   | 18                       |
| 19            | 8              | 9th         | 95%                   | All                      |

### Wisdom

| Ability Score | Magical Defense Adjustment | Bonus Spells | Chance of Spell Failure |
|---------------|----------------------------|--------------|-------------------------|
| 3             | -3                         | —            | 50%                     |
| 4             | -2                         | —            | 45%                     |
| 5             | -1                         | —            | 40%                     |
| 6             | -1                         | —            | 35%                     |
| 7             | -1                         | —            | 30%                     |
| 8             | 0                          | —            | 25%                     |
| 9             | 0                          | 0            | 20%                     |
| 10            | 0                          | 0            | 15%                     |
| 11            | 0                          | 0            | 10%                     |
| 12            | 0                          | 0            | 5%                      |
| 13            | 0                          | 1st          | 0%                      |
| 14            | 0                          | 1st          | 0%                      |
| 15            | +1                         | 2nd          | 0%                      |
| 16            | +2                         | 2nd          | 0%                      |
| 17            | +3                         | 3rd          | 0%                      |
| 18            | +4                         | 4th          | 0%                      |






### Charisma

### Table 6: Charisma

| Ability Score | Maximum # of Henchmen | Loyalty Base | Reaction Adjustment |
|---------------|-----------------------|--------------|---------------------|
| 3             | 1                     | -6           | -5                  |
| 4             | 1                     | -5           | -4                  |
| 5             | 2                     | -4           | -3                  |
| 6             | 2                     | -3           | -2                  |
| 7             | 3                     | -2           | -1                  |
| 8             | 3                     | -1           | 0                   |
| 9             | 4                     | 0            | 0                   |
| 10            | 4                     | 0            | 0                   |
| 11            | 4                     | 0            | 0                   |
| 12            | 5                     | 0            | 0                   |
| 13            | 5                     | 0            | +1                  |
| 14            | 6                     | +1           | +2                  |
| 15            | 7                     | +3           | +3                  |
| 16            | 8                     | +4           | +5                  |
| 17            | 10                    | +6           | +6                  |
| 18            | 15                    | +8           | +7                  |
| 19            | 20                    | +10          | +8                  |

## Races
***
### Human
***
### Dwarf
+1 Constitution, -1 Charisma at character creation. A character of the dwarven 
race can be a cleric, a fighter, or a thief. He can also choose to be a fighter/
cleric or fighter/thief.

Dwarves automatically know Common and Dwarvish. If bonus languages are used, a
dwarf may learn gnome, goblin, kobold, orc, and hobgoblin.

Dwarves gain a Constitution-based bonus on saving throws against poison and 
magical wands, staves, rods, and spells.

| Constitution Score | Saving Throw Bonus |
|--------------------|--------------------|
| 4-6                | +1                 |
| 7-10               | +2                 |
| 11-13              | +3                 |
| 14-17              | +4                 |
| 18-19              | +5                 |

All magical items that are not specifically suited to the character’s class have 
a 20 percent chance to malfunction when used by a dwarf. Dwarven infravision 
enables them to see up to 60 feet in the dark.  

| Chance  | Ability                                     |
|---------|---------------------------------------------|
| 5/6     | Detect grade or slope in passage            |
| 5/6     | Detect new tunnel or passage construction   |
| 4/6     | Detect sliding or shifting walls or rooms   |
| 3/6     | Detect stonework traps, pits, and deadfalls |
| 3/6     | Determine approximate depth underground     |

In melee, dwarves gain a +1 bonus to attack rolls against goblins, hobgoblins, 
and orcs.

Because of their small size and combat experience against larger enemies, giants,
ogres, trolls, and titans suffer a -4 penalty to attack rolls against dwarves.

Dwarven infravision enables them to see up to 60 feet in the dark.

***
### Elf

+1 Dexterity, -1 Constitution at character creation. A character of the elven
race can be a cleric, fighter, mage, thief, or ranger. An elf can also choose
to be a fighter/mage, fighter/thief, mage/thief, or fighter/mage/thief.

Elves automatically know Common and Elvish. If bonus languages are used, an elf
may learn gnome, halfling, goblin, hobgoblin, orc, and gnoll.

Elves have a 90 percent resistance to sleep and all charm-related spells. In
addition, they receive a +2 bonus on saving throws against charm effects.

When using a bow of any type, or a short or long sword, elves gain a +1 bonus
to attack rolls.

Elven infravision enables them to see up to 60 feet in the dark.

Elves are difficult to surprise when alone or accompanied only by elves and/or
halflings, provided they are not wearing metal armor. Under these conditions,
opponents suffer a -4 penalty to surprise rolls. If an elf must open a door or
screen before attacking, the penalty is reduced to -2.

Merely passing within 10 feet of a concealed door gives an elf a 1-in-6 chance
to notice it. Active searches improve the chance to find concealed doors and
allow elves to discover secret doors.

| Chance  | Ability                                        |
|---------|------------------------------------------------|
| 1/6 | Detect concealed door while passing within 10 feet |
| 2/6 | Detect concealed door during active search         |
| 2/6 | Detect secret door during active search            |

Elves require only 4 hours of rest each day.  

***
### Gnome

+1 Intelligence, -1 Wisdom at character creation. A character of the gnomish
race can be a fighter, thief, illusionist, fighter/thief, illusionist/thief,
or fighter/illusionist.

Gnomes automatically know Common, Dwarvish, and Gnomish. If bonus languages
are used, a gnome may learn halfling, goblin, kobold, and the simple speech
of burrowing mammals such as moles, badgers, weasels, shrews, and ground
squirrels.

Like dwarves, gnomes gain a Constitution-based bonus on saving throws against
magical wands, staves, rods, and spells.

| Constitution Score | Saving Throw Bonus |
|--------------------|--------------------|
| 4-6                | +1                 |
| 7-10               | +2                 |
| 11-13              | +3                 |
| 14-17              | +4                 |
| 18-19              | +5                 |

All magical items that are not specifically suited to the character's class
have a 20 percent chance to malfunction when used by a gnome. This does not
normally apply to magical weapons, armor, shields, illusionist items, or items
that duplicate thieving abilities.

In melee, gnomes gain a +1 bonus to attack rolls against kobolds and goblins.

Because of their small size and combat skill against larger foes, giants,
bugbears, ogres, trolls, ogre magi, and titans suffer a -4 penalty to attack
rolls against gnomes.

Gnomish infravision enables them to see up to 60 feet in the dark.

Being tunnel dwellers of exceptional merit, gnomes can detect the following
when underground. The gnome must stop and concentrate for one round to use any
of these abilities.

| Chance | Ability |
|---------|---------|
| 5/10 | Detect approximate direction |
| 5/10 | Detect grade or slope in passage |
| 7/10 | Detect unsafe walls, ceiling, or floors |
| 4/10 | Determine approximate depth underground |
| 3/6 | Determine approximate direction underground |

Gnome characters gain a +1 bonus to Intelligence scores. They suffer a -1
penalty to Wisdom because their curiosity often leads them unknowingly into
danger.


***
### Half-Elf

A character of the half-elven race can be a cleric, druid, fighter, ranger, mage,
specialist wizard, thief, or bard. A half-elf can also choose from the following 
multiclass combinations: cleric/fighter, cleric/ranger, cleric/mage, cleric/thief,
druid/fighter, druid/ranger, fighter/mage, fighter/thief, fighter/mage/thief,
and mage/thief.

They automatically know Common and Elvish. If bonus languages are used, they may
learn gnome, halfling, goblin, hobgoblin, orc, and gnoll.

Half-elves have a 30 percent resistance to sleep and all charm-related spells.

Half-elven infravision enables them to see up to 60 feet in the dark.

Merely passing within 10 feet of a concealed door gives a half-elf a 1-in-6
chance to notice it. Active searches improve the chance to find concealed doors
and allow half-elves to discover secret doors.

| Chance  | Ability                                            |
|---------|----------------------------------------------------|
| 1/6     | Detect concealed door while passing within 10 feet |
| 2/6     | Detect concealed door during active search         |
| 2/6     | Detect secret door during active search            |

***
### Halfling

-1 Strength, +1 Dexterity at character creation. A character of the halfling
race can be a cleric, fighter, thief, or fighter/thief.

Halflings automatically know Common and Halfling. If bonus languages are used,
a halfling may learn Dwarvish, Elvish, Gnomish, Goblin, Orc.

Like dwarves and gnomes, halflings gain a Constitution-based bonus on saving
throws against poison and magical wands, staves, rods, and spells.

| Constitution Score | Saving Throw Bonus |
|--------------------|--------------------|
| 4-6                | +1                 |
| 7-10               | +2                 |
| 11-13              | +3                 |
| 14-17              | +4                 |
| 18-19              | +5                 |

Halflings are natural marksmen. They gain a +1 bonus to attack rolls when using
thrown weapons and slings.

A halfling gains a bonus to surprise opponents while outdoors. The halfling
must be alone, or accompanied only by other halflings, and at least 90 feet
from any non-halfling. Under these conditions, opponents suffer a -4 penalty
to surprise rolls. If a door or other screen must be opened before attacking,
the penalty is reduced to -2.

Halflings have infravision, allowing them to see in the dark.

| Light Conditions       | Range |
|------------------------|--------|
| Normal Infravision     | 60 ft |
| Starlight or Moonlight | 90 ft |

Halflings possess a natural talent for judging terrain and underground
conditions. The halfling must concentrate on the desired information while
excluding all other distractions.

| Chance | Ability |
|---------|---------|
| 1/6 | Determine whether a passage slopes upward or downward |
| 2/6 | Determine approximate direction underground           |
| 2/6 | Determine approximate depth underground               |

***
### Racial Adjustments

### Table 8: Racial Ability Adjustments


## Classes
### Warrior

The warrior group encompasses the character classes whose primary role is
combat: fighters, paladins, and rangers.

Warriors can wear any type of armor and use the widest selection of weapons.
They gain 1d10 hit points per level from levels 1 through 9. After 9th level,
warriors gain a fixed 3 hit points per level and no longer receive additional
Constitution hit point bonuses.

Warriors gain multiple attacks as they advance in level.

| Warrior Level | Attacks Per Round |
|---------------|-------------------|
| 1-6           | 1/round           |
| 7-12          | 3/2 rounds        |
| 13+           | 2/round           |

### Warrior Experience Levels

| Level | Fighter XP | Paladin/Ranger XP | Hit Dice |
|--------|------------|------------------|----------|
| 1      | 0          | 0                | d10      |
| 2      | 2,000      | 2,250            | 2        |
| 3      | 4,000      | 4,500            | 3        |
| 4      | 8,000      | 9,000            | 4        |
| 5      | 16,000     | 18,000           | 5        |
| 6      | 32,000     | 36,000           | 6        |
| 7      | 64,000     | 75,000           | 7        |
| 8      | 125,000    | 150,000          | 8        |
| 9      | 250,000    | 300,000          | 9        |
| 10     | 500,000    | 600,000          | 9+3      |
| 11     | 750,000    | 900,000          | 9+6      |
| 12     | 1,000,000  | 1,200,000        | 9+9      |
| 13     | 1,250,000  | 1,500,000        | 9+12     |
| 14     | 1,500,000  | 1,800,000        | 9+15     |
| 15     | 1,750,000  | 2,100,000        | 9+18     |
| 16     | 2,000,000  | 2,400,000        | 9+21     |
| 17     | 2,250,000  | 2,700,000        | 9+24     |
| 18     | 2,500,000  | 3,000,000        | 9+27     |
| 19     | 2,750,000  | 3,300,000        | 9+30     |
| 20     | 3,000,000  | 3,600,000        | 9+33     |

#### Fighter
A fighter with a strength score greater than 15 receives a 10% experience bonus.

**Weapon Specialization**
: Weapon specialization is available only to single-class fighters. By devoting
additional weapon proficiency slots to a specific weapon, a fighter gains
greater accuracy, increased damage, and additional attacks with that weapon.

To specialize, a fighter must first be proficient with the weapon and then spend
one additional weapon proficiency slot. 

A fighter specialized in a melee weapon gains +1 to attack and +2 to damage 
rolls. This costs 1 extra profficiency.

A fighter specialized with a bow gains a point-blank range (6-30') that adds +2
to attack rolls. This costs 2 extra profficiency slots.

A fighter specialized in crossbows gain a point-blank range (6-60') that adds +2
to attack rolls. This costs 1 extra profficiency slot.

If the bow or crossbow is ready at the start of a round, they may fire before 
initiative rolls.

Specialist Attacks Per Round

| Level | Melee Weapon | Light Crossbow | Heavy Crossbow | Thrown Dagger | Thrown Dart | Other Missile Weapon |
|-------|--------------|----------------|----------------|---------------|-------------|----------------------|
| 1-6   | 3/2          | 1              | 1              | 3             | 3           | 3/2                  |
| 7-12  | 2            | 1              | 1              | 4             | 4           | 2                    |
| 13+   | 5/2          | 2              | 3/2            | 5             | 5           | 5/2                  |

These attack rates replace the normal warrior attacks per round while the
specialized weapon is being used.

#### Paladin

A paladin is a heroic human warrior dedicated to the cause of law and good.

| Ability Requirement | Minimum Score |
|---------------------|---------------|
| Strength            | 12            |
| Constitution        | 9             |
| Wisdom              | 13            |
| Charisma            | 17            |

A paladin with Strength and Charisma scores of 16 or greater gains a 10%
bonus to earned experience points.

Paladins must always remain Lawful Good. A paladin who knowingly commits an
evil act immediately and irrevocably loses paladin status and all special
abilities. A paladin who changes alignment loses paladin abilities until
atonement is made.

A paladin receives the following benefits:

| Ability              | Effect                                                |
|----------------------|-------------------------------------------------------|
| Detect Evil          | Concentrate to detect evil intent up to 60 feet away  |
| Divine Saving Throws | +2 bonus to all saving throws                         |
| Disease Immunity     | Immune to all forms of disease                        |
| Lay on Hands         | Heal 2 hit points per experience level per day        |
| Cure Disease         | Cure disease once per week for every 5 levels         |
| Protection from Evil | Constant 10-foot radius aura                          |
| Turn Undead          | Functions as a cleric two levels lower                |
| Holy Sword           | Projects a dispel magic effect when wielded           |

A paladin's aura of protection causes evil creatures within 10 feet to suffer
a -1 penalty to attack rolls.

A paladin can call a special warhorse upon reaching 4th level. The mount is an
exceptional and loyal creature that serves only the paladin.

A paladin can cast priest spells beginning at 9th level. Paladin spells are
drawn from the spheres of combat, divination, healing, and protection.

Paladin Spell Progression:

| Paladin Level | 1st | 2nd | 3rd | 4th |
|---------------|-----|-----|-----|-----|
| 9             | 1   | -   | -   | -   |
| 10            | 2   | -   | -   | -   |
| 11            | 2   | 1   | -   | -   |
| 12            | 2   | 2   | -   | -   |
| 13            | 2   | 2   | 1   | -   |
| 14            | 3   | 2   | 1   | -   |
| 15            | 3   | 2   | 1   | 1   |
| 16            | 3   | 3   | 2   | 1   |
| 17            | 3   | 3   | 2   | 2   |
| 18            | 3   | 3   | 3   | 2   |
| 19            | 3   | 3   | 3   | 3   |
| 20            | 3   | 3   | 3   | 3   |

Paladins may never possess more than 10 magical items. No more than one suit
of magical armor, one magical shield, four magical weapons, and four other
magical items may be owned at one time.

A paladin never retains wealth beyond what is necessary to maintain a modest
lifestyle, support followers, maintain a stronghold, and continue good works.
Excess wealth must be donated to lawful good causes.

A paladin attracts loyal followers and men-at-arms upon reaching 9th level and
establishing a stronghold.

A paladin may employ only lawful good henchmen and followers. Associates of
other good alignments are tolerated only when they actively pursue good causes.

#### Ranger

Rangers can be human, elf, or half-elf.

| Ability Requirement | Minimum Score |
|---------------------|---------------|
| Strength            | 13            |
| Dexterity           | 13            |
| Constitution        | 14            |
| Wisdom              | 14            |

A ranger with Strength, Dexterity, and Wisdom scores of 16 or greater gains a
10% bonus to earned experience points.

Rangers may be Lawful Good, Neutral Good, Chaotic Good, Lawful Neutral, True
Neutral, or Chaotic Neutral.

Rangers can use any weapon and wear any armor.

When wearing studded leather armor or lighter, a ranger can fight with two
weapons with no penalty to attack rolls.

Rangers are skilled woodsmen and automatically possess the Tracking ability
without spending proficiency slots. Their tracking ability improves as they
gain levels. When tracking, a ranger gains a +1 per 3 levels attained.

Rangers gain a +4 attack bonus against a chosen species of giant-class enemy.
Typical giant-class enemies include: Giants, Ogres, Trolls, Ettins, Titans

Rangers are skilled with animals and can attempt to calm, befriend, or train
domestic and wild animals through careful handling and observation.

A ranger can identify general traits and temperament in domestic animals and
may influence an animal's reaction with successful interaction.

Rangers can cast priest spells beginning at 8th level. Ranger spells are drawn
from the spheres of animal and plant.

Ranger Abilities

| Ranger Level | Hide in Shadows | Move Silently | Casting Level | 1st | 2nd | 3rd |
|--------------|----------------|----------------|---------------|-----|-----|-----|
| 1            | 10%            | 15%            | -             | -   | -   | -   |
| 2            | 15%            | 21%            | -             | -   | -   | -   |
| 3            | 20%            | 27%            | -             | -   | -   | -   |
| 4            | 25%            | 33%            | -             | -   | -   | -   |
| 5            | 31%            | 40%            | -             | -   | -   | -   |
| 6            | 37%            | 47%            | -             | -   | -   | -   |
| 7            | 43%            | 55%            | -             | -   | -   | -   |
| 8            | 49%            | 62%            | 1             | 1   | -   | -   |
| 9            | 56%            | 70%            | 2             | 2   | -   | -   |
| 10           | 63%            | 78%            | 3             | 2   | 1   | -   |
| 11           | 70%            | 86%            | 4             | 2   | 2   | -   |
| 12           | 77%            | 94%            | 5             | 2   | 2   | 1   |
| 13           | 85%            | 99%*           | 6             | 3   | 2   | 1   |
| 14           | 93%            | 99%*           | 7             | 3   | 2   | 2   |
| 15           | 99%*           | 99%*           | 8             | 3   | 3   | 2   |
| 16           | 99%*           | 99%*           | 9             | 3   | 3** | 3   |

\* Maximum percentile score  
\** Maximum spell ability

A ranger must always strive to remain good. If a ranger intentionally commits
an evil act, the ranger loses all ranger abilities and becomes a fighter of the
same level. Ranger status can be regained only through atonement and the
correction of the wrongdoing.

***
### Wizard

Arcane spells require study, preparation, and access to a spellbook.

Wizard Experience Levels

| Level | Experience Points | Hit Dice |
|-------|-------------------|----------|
| 1     | 0                 | d4       |
| 2     | 2,500             | 2        |
| 3     | 5,000             | 3        |
| 4     | 10,000            | 4        |
| 5     | 20,000            | 5        |
| 6     | 40,000            | 6        |
| 7     | 60,000            | 7        |
| 8     | 90,000            | 8        |
| 9     | 135,000           | 9        |
| 10    | 250,000           | 10       |
| 11    | 375,000           | 10+1     |
| 12    | 750,000           | 10+2     |
| 13    | 1,125,000         | 10+3     |
| 14    | 1,500,000         | 10+4     |
| 15    | 1,875,000         | 10+5     |
| 16    | 2,250,000         | 10+6     |
| 17    | 2,625,000         | 10+7     |
| 18    | 3,000,000         | 10+8     |
| 19    | 3,375,000         | 10+9     |
| 20    | 3,750,000         | 10+10    |


Wizard Spell Progression

| Wizard Level | 1st | 2nd | 3rd | 4th | 5th | 6th | 7th | 8th | 9th |
|--------------|-----|-----|-----|-----|-----|-----|-----|-----|-----|
| 1            | 1   | -   | -   | -   | -   | -   | -   | -   | -   |
| 2            | 2   | -   | -   | -   | -   | -   | -   | -   | -   |
| 3            | 2   | 1   | -   | -   | -   | -   | -   | -   | -   |
| 4            | 3   | 2   | -   | -   | -   | -   | -   | -   | -   |
| 5            | 4   | 2   | 1   | -   | -   | -   | -   | -   | -   |
| 6            | 4   | 2   | 2   | -   | -   | -   | -   | -   | -   |
| 7            | 4   | 3   | 2   | 1   | -   | -   | -   | -   | -   |
| 8            | 4   | 3   | 3   | 2   | -   | -   | -   | -   | -   |
| 9            | 4   | 3   | 3   | 2   | 1   | -   | -   | -   | -   |
| 10           | 4   | 4   | 3   | 2   | 2   | -   | -   | -   | -   |
| 11           | 4   | 4   | 4   | 3   | 3   | -   | -   | -   | -   |
| 12           | 4   | 4   | 4   | 4   | 4   | 1   | -   | -   | -   |
| 13           | 5   | 4   | 4   | 4   | 4   | 2   | -   | -   | -   |
| 14           | 5   | 5   | 4   | 4   | 4   | 2   | 1   | -   | -   |
| 15           | 5   | 5   | 5   | 4   | 4   | 2   | 1   | -   | -   |
| 16           | 5   | 5   | 5   | 5   | 4   | 3   | 2   | -   | -   |
| 17           | 5   | 5   | 5   | 5   | 5   | 3   | 2   | 1   | -   |
| 18           | 5   | 5   | 5   | 5   | 5   | 4   | 3   | 2   | -   |
| 19           | 5   | 5   | 5   | 5   | 5   | 4   | 4   | 2   | 1   |
| 20           | 5   | 5   | 5   | 5   | 5   | 5   | 4   | 3   | 2   |

A wizard's Intelligence determines the highest spell level that can be learned,
the chance to learn spells, and the maximum number of spells that can be known
per spell level.

Wizards begin play with a spellbook containing a small number of 1st-level
spells. Additional spells are acquired through research, instruction, copying,
or adventuring.

Arcane magic is divided into schools of magic:

- Abjuration
- Alteration
- Conjuration/Summoning
- Divination
- Enchantment/Charm
- Illusion/Phantasm
- Invocation/Evocation
- Necromancy

A mage studies all schools of magic. A specialist wizard focuses on a single
school of magic, gaining advantages within that school while giving up access
to one or more opposing schools.

The nine schools of magic are: Abjuration, Alteration, Conjuration/Summoning,
Enchantmen/Charm, Greater Divination, Illusion, Invocation/Evocation, Necromancy,
and Lesser Divination.

#### Specialist Wizard

A specialist wizard concentrates on a single school of magic.

A specialist gains the following benefits:

- One additional spell slot per spell level, provided the additional spell is
  from the specialist's chosen school.
- A +15% bonus when learning spells from the specialist school.
- A -15% penalty when learning spells from schools outside the specialty.
- A +1 bonus to saving throws against spells from the specialist school.
- Opponents suffer a -1 penalty on saving throws against the specialist's
  spells from the specialty school.
- When researching new spells, spells from the specialist school are treated
  as one level lower in difficulty.

A specialist may learn and cast spells only from schools available to the
specialty. Opposing schools are prohibited.

Specialists cannot be multiclass characters except for gnome illusionists.

#### Illusionist

An illusionist is a specialist wizard devoted to the schools of illusion and
phantasm. Only gnomes may be multiclass illusionists.

| Ability Requirement | Minimum Score |
|---------------------|---------------|
| Dexterity           | 16            |
| Intelligence        | 15            |

A gnome illusionist with an Intelligence score of 16 or greater gains a 10%
bonus to earned experience points.

In addition to the normal specialist wizard benefits, illusionists gain the
following abilities:

- Receive a +1 bonus on saving throws against illusion spells.
- Opponents suffer a -1 penalty on saving throws against the illusionist's
  illusion spells.
- At 1st level, begin play knowing one additional illusion spell beyond the
  normal starting allotment, provided an additional illusion spell is
  available.
- Receive a +15% bonus when learning illusion spells.
- Receive a -15% penalty when learning spells from non-specialist schools.
- Researching illusion spells is easier than researching spells from other
  schools.

Illusionists may learn spells only from the following schools:

- Alteration
- Conjuration/Summoning
- Greater Divination
- Illusion/Phantasm

At 1st level, an illusionist automatically knows the spell *Color Spray*.

### Priest

The priest group consists of clerics and druids.

All priests possess the following abilities:

- Cast priest spells.
- Perform priestly duties on behalf of their faith.
- Act as representatives of their deity or philosophy.
- Gain magical powers through divine favor.

Priests are restricted in their choice of weapons and armor according to the
requirements of their faith.

Priest spells are divided into spheres. A priest can cast spells only from the
major and minor spheres granted by his deity or doctrine.

Priest Experience Levels

| Level | Cleric XP | Druid XP | Hit Dice |
|-------|-----------|----------|----------|
| 1     | 0         | 0        | d8       |
| 2     | 1,500     | 2,000    | 2        |
| 3     | 3,000     | 4,000    | 3        |
| 4     | 6,000     | 7,500    | 4        |
| 5     | 13,000    | 12,500   | 5        |
| 6     | 27,500    | 20,000   | 6        |
| 7     | 55,000    | 35,000   | 7        |
| 8     | 110,000   | 60,000   | 8        |
| 9     | 225,000   | 90,000   | 9        |
| 10    | 450,000   | 125,000  | 9+2      |
| 11    | 675,000   | 200,000  | 9+4      |
| 12    | 900,000   | 300,000  | 9+6      |
| 13    | 1,125,000 | 750,000  | 9+8      |
| 14    | 1,350,000 | 1,500,000| 9+10     |
| 15    | 1,575,000 | 3,000,000| 9+12     |
| 16    | 1,800,000 | 3,500,000| 9+14     |
| 17    | 2,025,000 | 1,000,000| 9+16     |
| 18    | 2,250,000 | 1,500,000| 9+18     |
| 19    | 2,475,000 | 2,000,000| 9+20     |
| 20    | 2,700,000 | 2,000,000| 9+22     |

Priest Spell Progression

| Priest Level | 1st | 2nd | 3rd | 4th | 5th | 6th* | 7th** |
|--------------|-----|-----|-----|-----|-----|------|-------|
| 1            | 1   | -   | -   | -   | -   | -    | -     |
| 2            | 2   | -   | -   | -   | -   | -    | -     |
| 3            | 2   | 1   | -   | -   | -   | -    | -     |
| 4            | 3   | 2   | -   | -   | -   | -    | -     |
| 5            | 3   | 3   | 1   | -   | -   | -    | -     |
| 6            | 3   | 3   | 2   | -   | -   | -    | -     |
| 7            | 3   | 3   | 2   | 1   | -   | -    | -     |
| 8            | 3   | 3   | 3   | 2   | -   | -    | -     |
| 9            | 4   | 4   | 3   | 2   | 1   | -    | -     |
| 10           | 4   | 4   | 3   | 3   | 2   | -    | -     |
| 11           | 5   | 4   | 4   | 3   | 2   | 1    | -     |
| 12           | 6   | 5   | 5   | 3   | 2   | 2    | -     |
| 13           | 6   | 6   | 6   | 4   | 2   | 2    | -     |
| 14           | 6   | 6   | 6   | 5   | 3   | 2    | 1     |
| 15           | 7   | 7   | 6   | 6   | 4   | 2    | 1     |
| 16           | 7   | 7   | 7   | 6   | 5   | 3    | 1     |
| 17           | 8   | 8   | 7   | 7   | 5   | 3    | 2     |
| 18           | 8   | 8   | 8   | 7   | 6   | 4    | 2     |
| 19           | 9   | 9   | 8   | 8   | 6   | 4    | 2     |
| 20           | 9   | 9   | 9   | 8   | 7   | 5    | 2     |

\* Usable only by priests with Wisdom 17 or higher.  
\** Usable only by priests with Wisdom 18 or higher.

Priest spells are organized into sixteen spheres:

- All
- Animal
- Astral
- Charm
- Combat
- Creation
- Divination
- Elemental
- Guardian
- Healing
- Necromantic
- Plant
- Protection
- Summoning
- Sun
- Weather

Access to each sphere is granted as either:

| Access Type | Effect                                                   |
|-------------|----------------------------------------------------------|
| Major       | Cast all spells from the sphere                          |
| Minor       | Cast only 1st, 2nd, and 3rd level spells from the sphere |

The spheres available to a priest are determined by the deity, faith, or
philosophy the priest serves.

Priests receive their spells through prayer, meditation, and devotion. Unlike
wizards, priests do not maintain spell books and do not learn spells through
study. A priest's ability to cast spells depends upon divine favor and the
continued observance of the tenets of the faith.

At higher levels, priests often establish temples, shrines, monasteries, or
other places of worship and attract followers devoted to their cause.


#### Druid

Druids can be human or half-elf.

| Ability Requirement | Minimum Score |
|---------------------|---------------|
| Wisdom              | 12            |
| Charisma            | 15            |

A druid with Wisdom and Charisma scores of 16 or greater gains a 10% bonus
to earned experience points.

Unlike other clerics, druids are forbidden to wear metal armor. A druid may
wear only leather armor and use wooden shields. Druids are restricted to the
following weapons: Club, Sickle, Dart, Spear, Dagger, Scimitar, Sling, Staff.

They have major access to the following spheres:

- All
- Animal
- Elemental
- Healing
- Plant
- Weather

Druids have minor access to:

- Divination

Druids can use magical items normally allowed to priests, except for magical
armor and weapons that cannot be used by druids.

They gain a +2 bonus on saving throws against fire and electrical attacks.

Druids possess a secret language known only to druids. This language is taught
as part of druidic training and does not require a proficiency slot.

As druids advance in level, they gain additional powers.

| Level | Ability                                                                              |
|-------|--------------------------------------------------------------------------------------|
| 3+    | Identify plants, animals, and pure water with perfect accuracy                       |
| 3+    | Pass through overgrown areas without leaving a trail or suffering movement penalties |
| 3+    | Identify woodland creatures and determine whether they are native to an area         |
| 7+    | Immune to charm spells cast by woodland creatures                                    |
| 7+    | Shapechange into a reptile, bird, or mammal up to three times per day                |
| 9+    | Pass through natural fire and electrical phenomena without harm (per class description) |

Druids can determine information about wilderness surroundings.

| Chance  | Ability                                           |
|---------|---------------------------------------------------|
| 1/6     | Identify the general state of natural water       |
| 1/6     | Determine whether plants are natural or unnatural |
| 2/6     | Identify the approximate age of a tree stand      |
| 2/6     | Determine whether an animal trail is recent       |

Beginning at 7th level, a druid gains the ability to shapechange into animal
form. Each form can be used only once per day. The druid may assume the form
of a reptile, bird, or mammal ranging in size from a small bird to a black
bear. While transformed, the druid acquires the movement, Armor Class,
attacks, and senses of the chosen form, but retains personal memories and
intelligence.

### Rogue

### Rogue Experience Levels

| Level | Thief/Bard XP | Hit Dice |
|-------|---------------|----------|
| 1     | 0             | d6       |
| 2     | 1,250         | 2        |
| 3     | 2,500         | 3        |
| 4     | 5,000         | 4        |
| 5     | 10,000        | 5        |
| 6     | 20,000        | 6        |
| 7     | 40,000        | 7        |
| 8     | 70,000        | 8        |
| 9     | 110,000       | 9        |
| 10    | 160,000       | 10       |
| 11    | 220,000       | 10+2     |
| 12    | 440,000       | 10+4     |
| 13    | 660,000       | 10+6     |
| 14    | 880,000       | 10+8     |
| 15    | 1,100,000     | 10+10    |
| 16    | 1,320,000     | 10+12    |
| 17    | 1,540,000     | 10+14    |
| 18    | 1,760,000     | 10+16    |
| 19    | 1,980,000     | 10+18    |
| 20    | 2,200,000     | 10+20    |

### Thief

A thief relies on agility, stealth, observation, and quick thinking to
overcome challenges. Thieves may be of any race and may be of any alignment
except lawful good.

| Ability Requirement | Minimum Score |
|---------------------|---------------|
| Dexterity           | 9             |

Prime Requisite: Dexterity.

A thief with Dexterity 16 or greater gains a 10% bonus to earned experience
points.

Thieves are limited to the following weapons: Club, Dagger, Dart, Hand Crossbow,
Knife, Lasso, Short Bow, Sling, Long Sword, Short Sword, or Staff.

Thieves may wear leather armor, padded armor, elven chain, and magical armor
specifically allowed to thieves.

Base Thieving Skills

| Skill              | Base Score |
|--------------------|------------|
| Pick Pockets       | 15%        |
| Open Locks         | 10%        |
| Find/Remove Traps  | 5%         |
| Move Silently      | 10%        |
| Hide in Shadows    | 5%         |
| Detect Noise       | 15%        |
| Climb Walls        | 60%        |
| Read Languages     | 0%         |

At 1st level, a thief receives 60 discretionary points to distribute among
thieving skills. At each level thereafter, the thief receives 30 additional
points.

No skill may exceed 95%, excluding racial and Dexterity adjustments.

Thieving Skill Dexterity Adjustments

| Dexterity | Pick Pockets | Open Locks | Find/Remove Traps | Move Silently | Hide in Shadows |
|-----------|--------------|------------|-------------------|---------------|-----------------|
| 9         | -15%         | -10%       | -10%              | -20%          | -10%            |
| 10        | -10%         |  -5%       | -10%              | -15%          |  -5%            |
| 11        |  -5%         |   0%       |  -5%              | -10%          |   0%            |
| 12-15     |   0%         |   0%       |   0%              |   0%          |   0%            |
| 16        |   0%         |  +5%       |   0%              |   0%          |   0%            |
| 17        |  +5%         | +10%       |   0%              |  +5%          |  +5%            |
| 18        | +10%         | +15%       |  +5%              | +10%          | +10%            |
| 19        | +15%         | +20%       | +10%              | +15%          | +15%            |

Thieving Skill Racial Adjustments

| Skill              | Dwarf | Elf  | Gnome | Half-Elf | Halfling |
|--------------------|-------|------|-------|----------|----------|
| Pick Pockets       |       |  +5% |       | +10%     |  +5%     |
| Open Locks         | +10%  |  -5% |  +5%  |          |  +5%     |
| Find/Remove Traps  | +15%  |      | +10%  |          |  +5%     |
| Move Silently      |       |  +5% |  +5%  |          | +10%     |
| Hide in Shadows    |       | +10% |  +5%  |  +5%     | +15%     |
| Detect Noise       |       |  +5% | +10%  |          |  +5%     |
| Climb Walls        | -10%  |      | -15%  |          | -15%     |
| Read Languages     |  -5%  |      |       |          |  -5%     |

Thieving Skill Armor Adjustments

| Skill              | No Armor | Elven Chain | Studded Leather | Padded/Leather |
|--------------------|----------|-------------|-----------------|----------------|
| Pick Pockets       |  +5%     | -20%        | -30%            | -40%           |
| Open Locks         |          |  -5%        | -10%            | -10%           |
| Find/Remove Traps  |          |  -5%        | -10%            | -20%           |
| Move Silently      | +10%     | -10%        | -20%            | -30%           |
| Hide in Shadows    |  +5%     | -10%        | -20%            | -40%           |
| Detect Noise       |          |  -5%        | -10%            | -20%           |
| Climb Walls        | +10%     | -20%        | -30%            | -60%           |
| Read Languages     |          |             |                 |                |

A thief attacking an unaware victim from behind gains +4 to hit and extra damage:

| Thief Level | Damage Multiplier |
|-------------|-------------------|
| 1-4         | x2                |
| 5-8         | x3                |
| 9-12        | x4                |
| 13+         | x5                |


Backstab cannot be used against creatures lacking a distinct rear facing, such
as many oozes, slimes, jellies, and similar creatures.

**Pick Pockets**
: Pick pockets allows a thief to remove small items from a victim without being
noticed. Failure does not automatically alert the victim. A separate check 
determines whether the attempt is detected.

**Open Locks**
: Open locks allows a thief to bypass padlocks, combination locks, puzzle locks,
and similar mechanisms using specialized tools.

**Find/Remove Traps**
: This skill allows a thief to locate and disable small mechanical traps,
primarily those associated with locks, treasure chests, doors, and similar
devices.

**Move Silently**
: Move silently measures the thief's ability to approach without being heard.
The thief must move carefully at approximately one-third normal movement speed.

**Hide in Shadows**
: Hide in shadows allows a thief to conceal himself in darkness or natural cover.
The ability requires suitable concealment and cannot normally be used while
being directly observed.

**Detect Noise**
: Detect noise allows a thief to hear faint sounds behind doors, around corners,
or through barriers. The thief must remain still and concentrate for one round.

**Climb Walls**
: Thieves can climb surfaces impossible for most characters. This ability allows
climbing sheer walls and similar surfaces without ropes or special equipment,
provided handholds are available.

**Read Languages**
: Read languages allows the thief to understand written material in unfamiliar
languages. The roll is made for each document. Success allows partial or complete
understanding of the text.

**Thieves' Cant**
: All thieves learn a secret language known as thieves' cant. Thieves' cant
consists of coded phrases, slang, gestures, and expressions used to exchange
information without attracting attention. The cant is suitable for discussing
jobs, fences, safe houses, contacts, rewards, and criminal activities.

**Use Scrolls**
: Beginning at 10th level, a thief can attempt to use wizard scrolls. The thief
reads the scroll and attempts to activate the contained spell despite lacking
formal magical training.

#### Bard

A bard can be human, elf, or half-elf.

| Ability Requirement | Minimum Score |
|---------------------|---------------|
| Dexterity           | 12            |
| Intelligence        | 13            |
| Charisma            | 15            |

A bard with Dexterity and Charisma scores of 16 or greater gains a 10% bonus
to earned experience points.

Bards may be of any alignment except lawful good. A bard must remain at least
partially neutral to successfully fulfill the role.

Bards can use any weapon and wear any armor, including chain mail, but cannot
use shields.

All bards are proficient singers, musicians, storytellers, poets, and
entertainers. They begin play knowing how to play one musical instrument. Each
additional musical instrument requires an additional proficiency slot.

Bards learn arcane magic in much the same way as wizards. They maintain
spellbooks and cast wizard spells. However, they are dabblers rather than
dedicated wizards and advance more slowly in magical ability.

A bard's Intelligence determines the highest spell level that can be learned,
the chance to learn spells, and the maximum number of spells known per spell
level.

Bard Spell Progression

| Bard Level | 1st | 2nd | 3rd | 4th | 5th | 6th |
|------------|-----|-----|-----|-----|-----|-----|
| 1          | 1   | -   | -   | -   | -   | -   |
| 2          | 1   | -   | -   | -   | -   | -   |
| 3          | 2   | -   | -   | -   | -   | -   |
| 4          | 2   | 1   | -   | -   | -   | -   |
| 5          | 3   | 1   | -   | -   | -   | -   |
| 6          | 3   | 2   | 1   | -   | -   | -   |
| 7          | 3   | 2   | 1   | -   | -   | -   |
| 8          | 3   | 3   | 1   | -   | -   | -   |
| 9          | 3   | 3   | 2   | -   | -   | -   |
| 10         | 3   | 3   | 2   | 1   | -   | -   |
| 11         | 3   | 3   | 3   | 1   | -   | -   |
| 12         | 3   | 3   | 3   | 2   | -   | -   |
| 13         | 3   | 3   | 3   | 2   | 1   | -   |
| 14         | 3   | 3   | 3   | 3   | 1   | -   |
| 15         | 3   | 3   | 3   | 3   | 2   | -   |
| 16         | 4   | 3   | 3   | 3   | 2   | 1   |
| 17         | 4   | 4   | 3   | 3   | 2   | 1   |
| 18         | 4   | 4   | 4   | 3   | 3   | 1   |
| 19         | 4   | 4   | 4   | 4   | 3   | 2   |
| 20         | 4   | 4   | 4   | 4   | 4   | 3   |

Bard Abilities

| Skill              | Base Score |
|--------------------|------------|
| Climb Walls        | 50%        |
| Detect Noise       | 20%        |
| Pick Pockets       | 10%        |
| Read Languages     |  5%        |

At 1st level, a bard receives 20 discretionary points to distribute among the
four bard abilities. At each level thereafter, the bard receives 15 additional
points to distribute.

No skill may exceed 95%.

**Climb Walls**
: A bard can climb steep surfaces without tools or ropes.

**Detect Noise**
: A bard can hear faint sounds behind doors, around corners, or through
barriers, but must remain still and concentrate for one round.

**Pick Pockets**
: A bard can perform sleight-of-hand tricks, palm small objects, entertain a
crowd, or quietly remove items from another person.

**Read Languages**
: A bard can decipher unfamiliar writing and understand fragments of unknown
languages. Success indicates partial or complete understanding of the text.

**Bardic Lore**
: Bards accumulate knowledge through travel, study, observation, and
storytelling. The DM may allow a bard to recognize legends, historical references,
notable persons, magical traditions, heraldry, songs, customs, or local folklore
that other characters would not know.

**Inspire and Influence**
: A bard can perform before an audience using music, poetry, storytelling,
oratory, comedy, or other performances. After hearing a successful performance,
listeners may have their attitudes improved by one reaction category.

Examples:

- Hostile → Unfriendly
- Unfriendly → Neutral
- Neutral → Friendly
- Friendly → Helpful

A successful performance generally requires the audience to understand the bard
and be willing to listen.

**Counter Song**
: A bard can use music or performance to oppose certain magical effects that rely
on sound or charm. The bard must continue performing while maintaining the effect.

**Inspire Courage**
: A bard can inspire allies through music, poetry, speeches, or songs. After at
least one full round of performance, allies gain one of the following benefits:

- +1 attack rolls
- +1 saving throws
- +2 morale

The effect lasts while the performance continues and for a short period
thereafter at the DM's discretion.

**Use Scrolls**
: As the bard gains levels and magical knowledge, the bard becomes increasingly
capable of understanding and employing magical writings. Bards can use wizard
scrolls according to the normal rules governing arcane spellcasting.


## Proficiencies

| Group   | Weapon Initial | Weapon per Level | Weapon Penalty | Nonweapon Initial | Nonweapon per Level |
|---------|----------------|------------------|----------------|-------------------|--------------------|
| Warrior | 4              | 3                | -2             | 3                 | 3                  |
| Wizard  | 1              | 6                | -5             | 4                 | 3                  |
| Priest  | 2              | 4                | -3             | 4                 | 3                  |
| Rogue   | 2              | 4                | -3             | 3                 | 4                  |


### Nonweapon Profficiencies

| Proficiency         | Classes | Slots | Ability      | Check | Description                           |
|:--------------------|:-------:|:-----:|:-------------|:-----:|:--------------------------------------|
| Agriculture         |   G     |   1   | Intelligence |   0   | Grow crops and manage farmland        |
| Ancient History     |  PRW    |   1   | Intelligence |  -1   | Recall ancient peoples and events     |
| Animal Handling     |   G     |   1   | Wisdom       |  -1   | Calm and control domesticated animals |
| Animal Lore         |   F     |   1   | Intelligence |   0   | Know animal habits and weaknesses     |
| Animal Training     |   G     |   1   | Wisdom       |   0   | Train animals for specific tasks      |
| Appraising          |   R     |   1   | Intelligence |   0   | Estimate value within 10-20%          |
| Armorer             |   F     |   2   | Intelligence |  -2   | Make, repair, and assess armor        |
| Artistic Ability    |   G     |   1   | Wisdom       |   0   | Create and evaluate artwork           |
| Astrology           |   PW    |   2   | Intelligence |   0   | Interpret stars and celestial omens   |
| Blacksmithing       |   G     |   1   | Strength     |   0   | Forge and repair metal items          |
| Blind-fighting      |   FR    |   2   | N/A          |  N/A  | Reduce penalties when sightless       |
| Bowyer/Fletcher     |   F     |   1   | Dexterity    |  -1   | Make and repair bows and arrows       |
| Brewing             |   G     |   1   | Intelligence |   0   | Brew beer, ale, and spirits           |
| Carpentry           |   G     |   1   | Strength     |   0   | Build and repair wooden items         |
| Charioteering       |   F     |   1   | Dexterity    |  +2   | Control chariots at speed             |
| Cobbling            |   G     |   1   | Dexterity    |   0   | Make and repair footwear              |
| Cooking             |   G     |   1   | Intelligence |   0   | Prepare meals and preserve food       |
| Dancing             |   G     |   1   | Dexterity    |   0   | Perform and judge dance               |
| Direction Sense     |   G     |   1   | Wisdom       |  +1   | Avoid becoming lost                   |
| Disguise            |   R     |   1   | Charisma     |  -1   | Alter appearance convincingly         |
| Endurance           |   F     |   2   | Constitution |   0   | Resist fatigue and forced marches     |
| Engineering         |   PW    |   2   | Intelligence |  -3   | Design structures and machines        |
| Etiquette           |   G     |   1   | Charisma     |   0   | Know proper social customs            |
| Fire-building       |   G     |   1   | Wisdom       |  -1   | Start and maintain fires              |
| Fishing             |   G     |   1   | Wisdom       |  -1   | Catch fish with common methods        |
| Forgery             |   R     |   1   | Dexterity    |  -1   | Create convincing documents           |
| Gaming              |   FR    |   1   | Charisma     |   0   | Play games of chance and skill        |
| Gem Cutting         |   RW    |   2   | Dexterity    |  -2   | Cut, shape, and value gems            |
| Healing             |   P     |   2   | Wisdom       |  -2   | Restore 1d3 hp/day with care          |
| Heraldry            |   G     |   1   | Intelligence |   0   | Recognize noble arms and lineage      |
| Herbalism           |   PW    |   2   | Intelligence |  -2   | Identify and prepare medicinal herbs  |
| Hunting             |   F     |   1   | Wisdom       |  -1   | Track and harvest game                |
| Juggling            |   R     |   1   | Dexterity    |  -1   | Keep multiple objects aloft           |
| Jumping             |   R     |   1   | Strength     |   0   | Increase jump distance                |
| Languages-Ancient   |   PW    |   1   | Intelligence |   0   | Read or speak ancient tongues         |
| Languages-Modern    |   G     |   1   | Intelligence |   0   | Read or speak modern languages        |
| Leatherworking      |   G     |   1   | Intelligence |   0   | Make and repair leather goods         |
| Local History       |   PR    |   1   | Charisma     |   0   | Know regional history and legends     |
| Mining              |   G     |   2   | Wisdom       |  -3   | Locate and extract ore deposits       |
| Mountaineering      |   F     |   1   | N/A          |  N/A  | Climb cliffs and mountain terrain     |
| Musical Instrument  |   PR    |   1   | Dexterity    |  -1   | Play one musical instrument           |
| Navigation          |  PFW    |   1   | Intelligence |  -2   | Determine route and position          |
| Pottery             |   G     |   1   | Dexterity    |  -2   | Make fired clay vessels               |
| Reading Lips        |   R     |   2   | Intelligence |  -2   | Understand speech from lip movement   |
| Reading/Writing     |   PW    |   1   | Intelligence |  +1   | Read and write known languages        |
| Religion            |   PW    |   1   | Wisdom       |   0   | Know faiths, rites, and dogma         |
| Riding, Airborne    |   G     |   2   | Wisdom       |  -2   | Ride and control flying mounts        |
| Riding, Land-Based  |   G     |   1   | Wisdom       |  +3   | Ride and manage ground mounts         |
| Rope Use            |   G     |   1   | Dexterity    |   0   | Knot, splice, and secure ropes        |
| Running             |   F     |   1   | Constitution |  -6   | Increase overland movement            |
| Seamanship          |   G     |   1   | Dexterity    |  +1   | Work and operate aboard ships         |
| Seamstress/Tailor   |   G     |   1   | Dexterity    |  -1   | Sew, mend, and alter clothing         |
| Set Snares          |   FR    |   1   | Dexterity    |  -1   | Build traps for small game            |
| Singing             |   G     |   1   | Charisma     |   0   | Perform vocal music                   |
| Spellcraft          |   PW    |   1   | Intelligence |  -2   | Identify spells and magic effects     |
| Stonemasonry        |   G     |   1   | Strength     |  -2   | Cut, shape, and lay stone             |
| Survival            |   F     |   2   | Intelligence |   0   | Find food, water, and shelter         |
| Swimming            |   G     |   1   | Strength     |   0   | Swim and stay afloat                  |
| Tightrope Walking   |   R     |   1   | Dexterity    |   0   | Walk narrow supports safely           |
| Tracking            |   F     |   2   | Wisdom       |   0   | Follow tracks across terrain          |
| Tumbling            |   R     |   1   | Dexterity    |   0   | Roll, dive, and avoid obstacles       |
| Ventriloquism       |   R     |   1   | Intelligence |  -2   | Project voice elsewhere               |
| Weaponsmithing      |   F     |   3   | Wisdom       |  -3   | Forge and repair weapons              |
| Weather Sense       |   G     |   1   | Wisdom       |  -1   | Predict weather 6 hours ahead         |
| Weaving             |   G     |   1   | Intelligence |  -1   | Create cloth and woven goods          |

G: General  
F: Fighter  
P: Priest  
R: Rogue  
W: Wizard

## Money

Coins represent the monetary system. 

**Value**
: 500CP = 50SP = 10EP = 5GP = 1PP = $500  
**Size**
:1-inch diameter, 1/10-inch thick, 50 per pound  
**Volume**
:1 cubic inch stacked, 2.5 cubic inches loose  

## Equipment
### Mundane Equipment

### Clothing

| Item                             | Cost  |
|----------------------------------|------:|
| Belt                             |  3 sp |
| Boots, Riding                    |  3 gp |
| Boots, Soft                      |  1 gp |
| Breeches                         |  2 sp |
| Cap, Hat                         |  1 sp |
| Cloak, Good Cloth                |  8 sp |
| Cloak, Fine Fur                  | 50 gp |
| Girdle                           |  3 gp |
| Gloves                           |  1 sp |
| Gown, Common                     | 12 sp |
| Hose                             |  2 gp |
| Knife Sheath                     |  3 cp |
| Mittens                          |  3 sp |
| Pin                              |  6 gp |
| Plain Brooch                     | 10 gp |
| Robe, Common                     |  9 sp |
| Robe, Embroidered                | 20 gp |
| Sandals                          |  5 cp |
| Sash                             |  2 sp |
| Shoes                            |  1 gp |
| Silk Jacket                      | 80 gp |
| Surcoat                          |  5 sp |
| Sword Scabbard, Hanger, Baldric  |  4 gp |
| Tabard                           |  6 sp |
| Toga, Coarse                     |  8 cp |
| Tunic                            |  8 sp |
| Vest                             |  6 sp |

### Daily Food and Lodging

| Item                                   | Cost  |
|----------------------------------------|------:|
| Ale (per gallon)                       |  2 sp |
| Banquet (per person)                   | 10 gp |
| Bread                                  |  5 cp |
| Cheese                                 |  4 sp |
| City Room, Common (per day)            |  7 sp |
| City Room, Common (per month)          | 20 gp |
| City Room, Poor (per day)              |  2 sp |
| City Room, Poor (per month)            | 60 sp |
| Common Wine (pitcher)                  |  2 sp |
| Eggs or Fresh Vegetables               |  1 cp |
| Grain and Stabling for Horse (daily)   |  5 sp |
| Honey                                  |  5 sp |
| Inn Lodging, Common (per day)          |  5 sp |
| Inn Lodging, Common (per week)         |  3 gp |
| Inn Lodging, Poor (per day)            |  5 cp |
| Inn Lodging, Poor (per week)           |  2 sp |
| Meat (per meal)                        |  1 sp |
| Meals, Good (per day)                  |  5 sp |
| Meals, Common (per day)                |  3 sp |
| Meals, Poor (per day)                  |  1 sp |
| Separate Room (per month)              |  2 gp |
| Small Beer (per gallon)                |  5 cp |
| Soup                                   |  5 cp |

### Household Provisioning

| Item                                      | Cost  |
|-------------------------------------------|------:|
| Barrel of Pickled Fish                    |  3 gp |
| Butter (per lb.)                          |  2 sp |
| Coarse Sugar (per lb.)                    |  1 gp |
| Dry Rations (per week)                    | 10 gp |
| Eggs (per 100)                            |  8 sp |
| Eggs (per 2 dozen)                        |  2 sp |
| Figs (per lb.)                            |  3 sp |
| Firewood (per day)                        |  1 cp |
| Herbs (per lb.)                           |  5 cp |
| Nuts (per lb.)                            |  1 gp |
| Raisins (per lb.)                         |  2 sp |
| Rice (per lb.)                            |  2 sp |
| Salt (per lb.)                            |  1 sp |
| Salted Herring (per 100)                  |  1 gp |
| Spice, Exotic (per oz.) saffron, clove    | 15 gp |
| Spice, Rare (per oz.) pepper, ginger      |  2 gp |
| Spice, Uncommon (per lb.) cinnamon        |  1 gp |
| Tun of Cider (250 gal.)                   |  8 gp |
| Tun of Good Wine (250 gal.)               | 20 gp |

### Tack and Harness

| Item                             | Cost     | Wgt (lb.) |
|----------------------------------|---------:|----------:|
| Barding, Chain                   |   500 gp | 70        |
| Barding, Full Plate              | 2,000 gp | 85        |
| Barding, Full Scale              | 1,000 gp | 75        |
| Barding, Half Brigandine         |   500 gp | 45        |
| Barding, Half Padded             |   100 gp | 25        |
| Barding, Half Scale              |   500 gp | 50        |
| Barding, Leather or Padded       |   150 gp | 60        |
| Bit and Bridle                   |    15 sp | 3         |
| Cart Harness                     |     2 gp | 10        |
| Halter                           |     5 cp | 1         |
| Horseshoes and Shoeing           |     1 gp | 10        |
| Saddle, Pack                     |     5 gp | 15        |
| Saddle, Riding                   |    10 gp | 35        |
| Saddle Bags                      |     4 gp | 8         |
| Saddle Bags, Small               |     3 gp | 5         |
| Saddle Blanket                   |     3 sp | 4         |
| Yoke, Horse                      |     5 gp | 15        |
| Yoke, Ox                         |     3 gp | 20        |

### Transport

| Item                        | Cost      |
|-----------------------------|----------:|
| Barge                       |    500 gp |
| Canoe, Small                |     30 gp |
| Canoe, War                  |     50 gp |
| Caravel                     | 10,000 gp |
| Carriage, Common            |    150 gp |
| Carriage, Coach, Ornamented |  7,000 gp |
| Chariot                     |    200 gp |
| Coaster                     |  5,000 gp |
| Cog                         | 10,000 gp |
| Curragh                     |     50 gp |
| Drakkar                     | 25,000 gp |
| Dromond                     | 15,000 gp |
| Galleon                     | 50,000 gp |
| Great Galley                | 30,000 gp |
| Knarr                       |  3,000 gp |
| Longship                    | 10,000 gp |
| Oar, Common                 |      2 gp |
| Oar, Galley                 |     10 gp |
| Raft or Small Keelboat      |    100 gp |
| Sail                        |      2 gp |
| Sedan Chair                 |    100 gp |
| Wagon or Cart Wheel         |      5 gp |

| Item                            | Cost      | Wgt (lb.) |
|---------------------------------|----------:|----------:|
| Backpack                        |      2 gp |         2 |
| Barrel, Small                   |      2 gp |        30 |
| Basket, Large                   |      3 sp |         1 |
| Basket, Small                   |      5 cp |         — |
| Bell                            |      1 gp |         — |
| Belt Pouch, Large               |      1 gp |         1 |
| Belt Pouch, Small               |      7 sp |       1/2 |
| Block and Tackle                |      5 gp |         5 |
| Bolt Case                       |      1 gp |         1 |
| Bucket                          |      5 sp |         3 |
| Chain (per ft.), Heavy          |      4 gp |         3 |
| Chain (per ft.), Light          |      3 gp |         1 |
| Chest, Large                    |      2 gp |        25 |
| Chest, Small                    |      1 gp |        10 |
| Cloth (per 10 sq. yds.), Common |      7 gp |        10 |
| Cloth (per 10 sq. yds.), Fine   |     50 gp |        10 |
| Cloth (per 10 sq. yds.), Rich   |    100 gp |        10 |
| Candle                          |      1 cp |         — |
| Canvas (per sq. yd.)            |      4 sp |         1 |
| Chalk                           |      1 cp |         * |
| Crampons                        |      4 gp |         2 |
| Fishhook                        |      1 sp |        ** |
| Fishing Net (10 ft. sq.)        |      4 sp |         5 |
| Flint and Steel                 |      5 sp |         * |
| Glass Bottle                    |     10 gp |         * |
| Grappling Hook                  |      8 sp |         4 |
| Holy Item (symbol, water, etc.) |     25 gp |         * |
| Hourglass                       |     25 gp |         1 |
| Iron Pot                        |      5 sp |         2 |
| Ladder (10 ft.)                 |      5 cp |        20 |
| Lantern, Beacon                 |    150 gp |        50 |
| Lantern, Bullseye               |     12 gp |         3 |
| Lantern, Hooded                 |      7 gp |         2 |
| Lock, Good                      |    100 gp |         1 |
| Lock, Poor                      |     20 gp |         1 |
| Magnifying Glass                |    100 gp |         * |
| Map or Scroll Case              |      8 sp |       1/2 |
| Merchant's Scale                |      2 gp |         1 |
| Mirror, Small Metal             |     10 gp |         * |
| Musical Instrument              |  5-100 gp |     1/2-3 |
| Oil (per Flask), Greek Fire     |     10 gp |         2 |
| Oil (per Flask), Lamp           |      6 cp |         1 |
| Paper (per Sheet)               |      2 gp |        ** |
| Papyrus (per Sheet)             |      8 sp |        ** |
| Parchment (per Sheet)           |      1 gp |        ** |
| Perfume (per Vial)              |      5 gp |        ** |
| Piton                           |      3 cp |       1/2 |
| Quiver                          |      8 sp |         1 |
| Rope (per 50 ft.), Hemp         |      1 gp |        20 |
| Rope (per 50 ft.), Silk         |     10 gp |         8 |
| Sack, Large                     |      2 sp |       1/2 |
| Sack, Small                     |      5 cp |         * |
| Sealing/Candle Wax (lb.)        |      1 gp |         1 |
| Sewing Needle                   |      5 sp |        ** |
| Signal Whistle                  |      8 sp |         * |
| Signet Ring, Personal Seal      |      5 gp |         * |
| Soap (per lb.)                  |      5 sp |         1 |
| Spyglass                        |  1,000 gp |         1 |
| Tent, Large                     |     25 gp |        20 |
| Tent, Pavilion                  |    100 gp |        50 |
| Tent, Small                     |      5 gp |        10 |
| Thieves' Picks                  |     30 gp |         1 |
| Torch                           |      1 cp |         1 |
| Water Clock                     |  1,000 gp |       200 |
| Whetstone                       |      2 cp |         1 |
| Wineskin                        |      8 sp |         1 |
| Winter Blanket                  |      5 sp |         3 |
| Writing Ink (per Vial)          |      8 gp |        ** |

```
*	Negligible weight
**	Practically weightless
```

### Animals

| Animal                    | Cost      |
|---------------------------|----------:|
| Boar                      |     10 gp |
| Bull                      |     20 gp |
| Calf                      |      5 gp |
| Camel                     |     50 gp |
| Capon                     |      3 cp |
| Cat                       |      1 sp |
| Chicken                   |      2 cp |
| Cow                       |     10 gp |
| Dog, Guard                |     25 gp |
| Dog, Hunting              |     17 gp |
| Dog, War                  |     20 gp |
| Donkey, Mule, or Ass      |      8 gp |
| Elephant, Labor           |    200 gp |
| Falcon (Trained)          |  1,000 gp |
| Goat                      |      1 gp |
| Goose                     |      5 cp |
| Guinea Hen                |      2 cp |
| Horse, Draft              |    200 gp |
| Horse, Heavy War          |    400 gp |
| Horse, Light War          |    150 gp |
| Horse, Medium War         |    225 gp |
| Horse, Riding             |     75 gp |
| Hunting Cat (Jaguar,etc.) |  5,000 gp |
| Ox                        |     15 gp |
| Partridge                 |      5 cp |
| Peacock                   |      5 sp |
| Pig                       |      3 gp |
| Pigeon                    |      1 cp |
| Pigeon, Homing            |    100 gp |
| Pony                      |     30 gp |
| Ram                       |      4 gp |
| Sheep                     |      2 gp |
| Songbird                  |     10 sp |
| Swan                      |      5 sp |




### Weapons

| Item                      | Cost   | Wgt | Size | Type | Spd | S Dam | L Dam |
|---------------------------|--------|-----|------|------|-----|-------|-------|
| Arquebus                  | 500 gp |  10 | M    | P    |  15 | 1d10  | 1d10  |
| Battle Axe                |   5 gp |   7 | M    | S    |   7 | 1d8   | 1d8   |
| Blowgun                   |   5 gp |   2 | L    | P    |   5 | —     | —     |
| Blowgun, Barbed Dart      |   1 sp |   * | S    | P    |   — | 1d3   | 1d2   |
| Blowgun, Needle           |   2 gp |   * | S    | —    |   — | 1     | 1     |
| Bow, Composite Long       | 100 gp |   3 | L    | —    |   7 | —     | —     |
| Bow, Composite Short      |  75 gp |   2 | M    | —    |   6 | —     | —     |
| Bow, Flight Arrow         |   3 sp |   * | S    | P    |   — | 1d6   | 1d6   |
| Bow, Long                 |  75 gp |   3 | L    | P    |   8 | —     | —     |
| Bow, Sheaf Arrow          |   3 sp |   * | S    | P    |   — | 1d8   | 1d8   |
| Bow, Short                |  30 gp |   2 | M    | —    |   7 | —     | —     |
| Club                      |      — |   3 | M    | B    |   4 | 1d6   | 1d3   |
| Crossbow, Hand Quarrel    |   1 gp |   * | S    | P    |   — | 1d3   | 1d2   |
| Crossbow, Hand            |  30 gp |   3 | S    | P    |   5 | —     | —     |
| Crossbow, Heavy Quarrel   |   2 sp |   * | S    | P    |   — | 1d4+1 | 1d6+1 |
| Crossbow, Heavy           |  50 gp |  14 | M    | —    |  10 | —     | —     |
| Crossbow, Light Quarrel   |   1 sp |   * | S    | P    |   — | 1d4   | 1d4   |
| Crossbow, Light           |  35 gp |   7 | M    | —    |   7 | —     | —     |
| Dagger                    |   2 gp |   1 | S    | P    |   2 | 1d4   | 1d3   |
| Dart                      |   5 sp | 1/2 | S    | P    |   2 | 1d3   | 1d2   |
| Footman's Flail           |  15 gp |  15 | M    | B    |   7 | 1d6+1 | 2d4   |
| Footman's Mace            |   8 gp |  10 | M    | B    |   7 | 1d6+1 | 1d6   |
| Footman's Pick            |   8 gp |   6 | M    | P    |   7 | 1d6+1 | 2d4   |
| Hand or Throwing Axe      |   1 gp |   5 | M    | S    |   4 | 1d6   | 1d4   |
| Harpoon                   |  20 gp |   6 | L    | P    |   7 | 2d4   | 2d4   |
| Horseman's Flail          |   8 gp |   5 | M    | B    |   6 | 1d4+1 | 1d4+1 |
| Horseman's Mace           |   5 gp |   6 | M    | B    |   6 | 1d6   | 1d4   |
| Horseman's Pick           |   7 gp |   4 | M    | P    |   5 | 1d4+1 | 1d4   |
| Javelin                   |   5 sp |   2 | S    | P    |   4 | 1d6   | 1d6   |
| Knife                     |   5 sp | 1/2 | S    | P/S  |   2 | 1d3   | 1d2   |
| Lance, Heavy Horse        |  15 gp |  15 | L    | P    |   8 | 1d8+1 | 3d6   |
| Lance, Jousting           |  20 gp |  20 | L    | P    |  10 | 1d3-1 | 1d2-1 |
| Lance, Light Horse        |   6 gp |   5 | L    | P    |   6 | 1d6   | 1d8   |
| Lance, Medium Horse       |  10 gp |  10 | L    | P    |   7 | 1d6+1 | 2d6   |
| Mancatcher                |  30 gp |   8 | L    | —    |   7 | —     | —     |
| Morning Star              |  10 gp |  12 | M    | B    |   7 | 2d4   | 1d6+1 |
| Polearm, Awl Pike         |   5 gp |  12 | L    | P    |  13 | 1d6   | 1d12  |
| Polearm, Bardiche         |   7 gp |  12 | L    | S    |   9 | 2d4   | 2d6   |
| Polearm, Bec de Corbin    |   8 gp |  10 | L    | P/B  |   9 | 1d8   | 1d10  |
| Polearm, Bill-Guisarme    |   7 gp |   8 | L    | P/S  |  10 | 2d4   | 1d10  |
| Polearm, Fauchard         |   8 gp |   7 | L    | P/S  |   8 | 1d6   | 1d8   |
| Polearm, Fauchard-Fork    |   8 gp |   9 | L    | P/S  |   8 | 1d8   | 1d10  |
| Polearm, Glaive           |   6 gp |   8 | L    | S    |   8 | 1d6   | 1d10  |
| Polearm, Glaive-Guisarme  |  10 gp |  10 | L    | P/S  |   9 | 2d4   | 2d6   |
| Polearm, Guisarme         |   5 gp |   8 | L    | S    |   8 | 2d4   | 1d8   |
| Polearm, Guisarme-Voulge  |   8 gp |  15 | L    | P/S  |  10 | 2d4   | 2d4   |
| Polearm, Halberd          |  10 gp |  15 | L    | P/S  |   9 | 1d10  | 2d6   |
| Polearm, Hook Fauchard    |  10 gp |   8 | L    | P/S  |   9 | 1d4   | 1d4   |
| Polearm, Lucern Hammer    |   7 gp |  15 | L    | P/B  |   9 | 2d4   | 2d4   |
| Polearm, Military Fork    |   5 gp |   7 | L    | P    |   7 | 1d8   | 2d4   |
| Polearm, Partisan         |  10 gp |   8 | L    | P    |   9 | 1d6   | 1d6+1 |
| Polearm, Ranseur          |   6 gp |   7 | L    | P    |   8 | 2d4   | 2d4   |
| Polearm, Spetum           |   5 gp |   7 | L    | P    |   8 | 1d6+1 | 2d6   |
| Polearm, Voulge           |   5 gp |  12 | L    | S    |  10 | 2d4   | 2d4   |
| Quarterstaff              |      — |   4 | L    | B    |   4 | 1d6   | 1d6   |
| Scourge                   |   1 gp |   2 | S    | —    |   5 | 1d4   | 1d2   |
| Sickle                    |   6 sp |   3 | S    | S    |   4 | 1d4+1 | 1d4   |
| Sling                     |   5 cp |   — | S    | —    |   6 | —     | —     |
| Sling Bullet              |   1 cp | 1/2 | S    | B    |   — | 1d4+1 | 1d4+1 |
| Sling Stone               |      — | 1/2 | S    | B    |   — | 1d4   | 1d4   |
| Spear                     |   8 sp |   5 | M    | P    |   6 | 1d6   | 1d8   |
| Staff Sling               |   2 sp |   2 | M    | B    |  11 | —     | —     |
| Bastard Sword             |  25 gp |  10 | M    | S    |   6 | 1d8   | 1d12  |
| Bastard Sword (2H)        |  25 gp |  10 | M    | S    |   8 | 2d4   | 2d8   |
| Broad Sword               |  10 gp |   4 | M    | S    |   5 | 2d4   | 1d6+1 |
| Khopesh                   |  10 gp |   7 | M    | S    |   9 | 2d4   | 1d8   |
| Long Sword                |  15 gp |   4 | M    | S    |   5 | 1d8   | 1d12  |
| Scimitar                  |  15 gp |   4 | M    | S    |   5 | 1d8   | 1d8   |
| Short Sword               |  10 gp |   3 | S    | S    |   3 | 1d6   | 1d8   |
| Two-Handed Sword          |  50 gp |  15 | L    | P    |  10 | 1d10  | 3d6   |
| Trident                   |  15 gp |   5 | M    | P    |   7 | 1d6+1 | 3d4   |
| Warhammer                 |   2 gp |   6 | M    | B    |   4 | 1d4+1 | 1d4   |
| Whip                      |   1 sp |   2 | M    | —    |   8 | 1d2   | 1     |











### Weapons
- Melee weapons
- Missile weapons
- Ammunition

## Combat
- Attack rolls and THAC0
- Initiative
- Movement in combat
- Melee and missile combat
- Nonlethal combat
- Saving throws
- Healing and injury

## Saving Throws
- Paralyzation, poison, death magic
- Rod, staff, wand
- Petrification, polymorph
- Breath weapon
- Spell

## Vision
- Visibility ranges
- Light sources
- Infravision

## Time & Movement
- Base movement rates
- Overland movement
- Swimming
- Climbing


### Table 65: Base Climbing Success Rates
| Character Type | Base Chance |
|:---------------|:-----------:|
| Thief (Climb Walls) | 60% |
| Bard | 50% |
| Unskilled Climber | 40% |

### Table 66: Climbing Modifiers
| Condition | Modifier |
|:----------|---------:|
| Abundant handholds | +55% |
| Rope against wall | +40% |
| Inward slope | +25% |
| Scale/Chain armor | -15% |
| Banded/Splint armor | -25% |
| Plate armor | -50% |
| Dwarf | -10% |
| Gnome/Halfling | -15% |
| Wet/Crumbling surface | -25% |
| Slippery surface | -40% |
| Encumbrance category | -5% |

### Table 67: Rates of Climbing
| Surface | Rate |
|:--------|:-----|
| Very smooth | 1/3 movement |
| Smooth/cracked | 1/2 movement |
| Rough | Full movement |
| Sloping wall | 2× movement |
| Rope & wall | Full movement |
| Thieves | Double listed rates |
| Rappelling | Normal movement |

## Note
A complete reconstruction of Tables 1-67 requires full OCR extraction of all table pages from the source PDF. The currently accessible OCR layer exposes only portions of Tables 1 and 65-67 and references to tables 2-64.
